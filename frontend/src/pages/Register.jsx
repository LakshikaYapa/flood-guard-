...
const Register = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:grid md:grid-cols-2 bg-slate-950">
      {/* Photo: a short banner on phones, full left-side panel on tablet/desktop */}
       <div
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="flex gap-3">
              <input type="text" placeholder={lang === 'en' ? 'First Name' : 'මුල් නම'} required
                className="w-1/2 p-2.5 bg-slate-900 border border-slate-800 rounded text-white text-sm"
            ...
            <input type="email" placeholder={lang === 'en' ? 'Email Address' : 'විද්‍යුත් තැපෑල'} required
              className="w-full p-2.5 bg-slate-900 border border-slate-800 rounded text-white text-sm"
            ...
            <div className="flex gap-3">
              <select className="w-1/2 p-2.5 bg-slate-900 border border-slate-800 rounded text-white text-sm"
                value={formData.province} onChange={handleProvinceChange} required>
                <option value="">{lang === 'en' ? 'Province' : 'පළාත'}</option>
                {provinces.map((p) => <option key={p} value={p}>{p}</option>)}
              </select>

              <select className="w-1/2 p-2.5 bg-slate-900 border border-slate-800 rounded text-white text-sm disabled:opacity-40"
